...
createArticleActions.addContributor = contributorEmail => (
  ...
  const contributor = users.find(user => user.email === contributorEmail);
...
createArticleActions.saveFormData = (title, content, section) => ({
  type: t.CREATE_ARTICLE_FORM.SAVE_FORM_DATA,
  payload: { title, content, section }
});

createArticleActions.clearFormData = (title, content, section) => ({
  type: t.CREATE_ARTICLE_FORM.CLEAR_FORM_DATA
});

createArticleActions.submitArticle = ({
  title,
  content,
  section,
  contributors
...
export const createArticle = ({ title, content, section }) => dispatch => {
  ...
  return (
    axios
      .post(`${STUY_SPEC_API_URL}/sections/${section}/articles`, article)
      .then(response => {
        dispatch({
...
editArticleActions.addContributor = contributorUsername => (
  ...
  const users = usersSelector(getState());
  const contributor = users.find(user => user.username === contributorUsername);
  const contributorId = contributor.id;
...
export const fetchArticles = () => dispatch => {
  ...
  axios
    .get(`${STUY_SPEC_API_URL}/articles`, AXIOS_CONFIG)
    .then(response => {
      dispatch({
...
export const deleteArticles = articleSlugs => dispatch => {
  dispatch({
  ...
  axios
    .all(
      articleSlugs.map(articleSlug =>
        axios.delete(`${STUY_SPEC_API_URL}/articles/${articleSlug}`)
      )
    )
    .then(response =>
      dispatch({
